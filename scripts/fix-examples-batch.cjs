...
const templates = {
  A: ex => ex.includes('can add a real detail to my essay about'),
  B: ex => ex.includes('when I write about') && ex.includes('in a Yunnan English essay'),
  C: ex => ex.includes('In a Yunnan English writing task about') && ex.includes('I can learn the word')
};

function isTemplate(ex) {
  return templates.A(ex) || templates.B(ex) || templates.C(ex);
}
...
const functionWordExamples = {
  ...
  'ought': [
    {en: 'You ought to be more careful.', cn: '你应该更小心一些。'}
  ],
  'used': [
    {en: 'I used to play basketball.', cn: '我过去常打篮球。'}
  ],
  'at': [
  ...
  'during': [
    {en: 'I slept well during the night.', cn: '我晚上睡得很好。'},
    {en: 'Be quiet during the class.', cn: '上课期间请保持安静。'}
  ],
  'before': [
...
function getTemplate(word, pos, meaning, semester, dayIndex, wordIndex) {
  ...
  if (posLower.includes('n.') && posLower.includes('v.')) {
    templateList = verbTemplates; // v./n. 优先用动词模板
    ...
    // unknown pos, default to noun
    templateList = nounTemplates;
  }
  
  // 使用组合哈希选择模板，确保同一词+学期+天使用固定模板，但不同词不同
...
console.log(`Fixed ${fixCount} template examples.`);
console.log(`Skipped ${skipCount} non-template examples.`);
